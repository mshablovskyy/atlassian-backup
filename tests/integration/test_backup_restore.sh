#!/usr/bin/env bash
# End-to-end integration test: backup a page tree, create a fresh space, restore into it.
#
# Usage:
...
BACKUP_URL="http://localhost:8090/spaces/ORIGINALCONTENT/pages/6032735/Content+for+validation"
...
# ── Step 2: Generate unique space identifiers ───────────────────────
...
# ── Step 3: Create target space (on recovery instance) ───────────────

step "Creating Confluence space: $SPACE_KEY on $RECOVERY_BASE"

HTTP_CODE="$(curl -s -o /tmp/e2e_create_space.json -w "%{http_code}" \
...
if [[ "$HTTP_CODE" -ne 200 ]]; then
    ...
    cat /tmp/e2e_create_space.json
    fail "Failed to create space (HTTP $HTTP_CODE)"
fi
ok "Space created (HTTP $HTTP_CODE)"

# ── Step 4: Run backup (from source instance) ────────────────────────

step "Running backup from source instance"

BACKUP_ARGS="--output-dir output --env-file $BACKUP_ENV_FILE --verbose"
if [[ "$NO_STORE_RAW_RESPONSE" == "true" ]]; then
    BACKUP_ARGS="$BACKUP_ARGS --no-store-raw-response"
fi
make backup URL="$BACKUP_URL" ARGS="$BACKUP_ARGS"
ok "Backup completed"

# ── Step 5: Find the latest backup directory ────────────────────────
...
# ── Step 6: Run restore (to recovery instance) ───────────────────────

step "Restoring backup to space $SPACE_KEY on $RECOVERY_BASE"

RESTORE_ARGS="--env-file $RECOVERY_ENV_FILE --verbose"
if [[ "$RESOLVE_USERKEYS" == "true" ]]; then
    RESTORE_ARGS="$RESTORE_ARGS --resolve-userkeys"
fi
make restore DIR="$BACKUP_DIR" SPACE="$SPACE_KEY" ARGS="$RESTORE_ARGS"
...
if [[ "$RESTORE_EXIT" -eq 0 ]]; then
    ...
    printf "\n\033[1;31mFAILED (restore had errors)\033[0m\n"
    exit 1
fi
